...
class SmartGenerator:
    ...
    def generate_ticket(
        ...
        """
        توليد تذكرة ذكية بالبحث الشامل حتى إيجاد تذكرة تطابق جميع الشروط
        
        استراتيجية البحث:
        1. التوليد العشوائي الذكي (غير محدود)
        2. التحسين التدريجي
        3. البحث الشامل عند الحاجة
        
        لن يتوقف حتى يجد تذكرة مطابقة أو يستنفذ جميع الاحتمالات الممكنة
        """
        ...
        # حساب إجمالي التركيبات الممكنة
        from math import comb
        total_combinations = comb(len(pool), remaining_size)
        
        # المرحلة 1: البحث العشوائي الذكي (بدون حد أقصى!)
        # نبحث حتى نجد أو نصل لعدد محاولات = عدد التركيبات الممكنة
        attempts = 0
        max_random_attempts = min(total_combinations, 1000000)  # حد أقصى مليون محاولة عشوائية
        ...
        while attempts < max_random_attempts:
            ...
            # فحص جميع الشروط
            if avoid_consecutive and self._has_consecutive(candidate):
                continue
        ...
        # المرحلة 2: البحث الشامل المنهجي (إذا لم ينجح العشوائي)
        # هذا للحالات الصعبة جداً - نفحص كل التركيبات الممكنة
        if total_combinations <= 100000:  # إذا كان العدد معقول
            from itertools import combinations
            
            for combo in combinations(pool, remaining_size):
                candidate = list(fixed_numbers) + list(combo)
                candidate.sort()
        ...
        # المرحلة 3: بحث شامل مع استرخاء تدريجي للشروط
        # إذا كانت الشروط مستحيلة رياضياً، نخبر المستخدم
        # لكن نحاول أولاً مع استرخاء بسيط جداً
        
        # محاولة أخيرة: استرخاء قيد الظلال بمقدار 1 فقط
        relaxed_shadow = max_shadow + 1
        attempts = 0
        ...
        while attempts < max_random_attempts:
            ...
            shadow_count = self._count_shadows(candidate)
            if shadow_count > relaxed_shadow:
                continue
            
            return candidate
        
        # إذا وصلنا هنا، الشروط قد تكون مستحيلة رياضياً
        return None
    
    def generate_ticket_exhaustive(
        self,
        ticket_size: int = 6,
        strategy: str = 'balanced',
        sum_target: Optional[int] = None,
        ...
        """
        البحث الشامل المطلق - يفحص كل التركيبات الممكنة
        
        استخدم هذه الدالة فقط للحالات الصعبة جداً
        تنبيه: قد تستغرق وقتاً طويلاً (دقائق أو ساعات حسب عدد التركيبات)
        """
        ...
        # فحصنا كل الاحتمالات - الشروط مستحيلة رياضياً
        return None
    ...
    def _count_shadows(self, nums: List[int]) -> int:
        ...
        for past_draw in self.analyzer.past_draws_sets:
            match = len(ticket_set & past_draw)
            if match > max_match:
                max_match = match
        return max_match
...
def render_smart_generator(analyzer: AdvancedAnalyzer, generator: SmartGenerator):
    ...
    with col_settings:
        ...
        avoid_consecutive = st.checkbox("تجنب المتتاليات", value=True)
        max_shadow = st.slider("الحد الأقصى للظلال", 0, 5, 2)
        
        # خيار البحث الشامل
        st.divider()
        exhaustive_search = st.checkbox(
            "🔍 البحث الشامل (موصى به للشروط الصارمة)",
            value=False,
            help="يفحص كل التركيبات الممكنة حتى يجد تذكرة تطابق الشروط بالضبط. قد يستغرق وقتاً أطول لكن يضمن الدقة."
        )
        
        if exhaustive_search:
            st.warning("""
            ⏱️ **ملاحظة**: البحث الشامل قد يستغرق من ثوانٍ إلى دقائق حسب:
            - عدد الأرقام في التذكرة
            - عدد الأرقام الثابتة
            - صرامة الشروط
            
            💪 **الميزة**: يضمن إيجاد التذاكر المطابقة للشروط أو يخبرك أن الشروط مستحيلة رياضياً.
            """)
        ...
        with st.expander("💡 نصائح للحصول على أفضل نتائج"):
            st.markdown("""
            - **الحد الأقصى للظلال (0-2)**: صارم جداً، قد يفشل التوليد
            - **الحد الأقصى للظلال (3-4)**: متوازن وموصى به ✅
            - **الحد الأقصى للظلال (5)**: مرن، سهل التوليد
            - **تجنب المتتاليات**: يجعل التوليد أصعب قليلاً
            - **الأرقام الثابتة**: كلما زادت كلما صعب التوليد
            - **استهداف متوسط محدد**: يقلل الخيارات المتاحة
            
            💡 **للحصول على نتائج سريعة**: استخدم الحد الأقصى للظلال = 3 أو 4
            """)
        ...
        if st.button("🎲 توليد", type="primary", use_container_width=True):
            ...
            if strategy == 'hot':
                pool_size = len(analyzer.hot_pool - fixed_numbers)
            elif strategy == 'cold':
                pool_size = len(analyzer.cold_pool - fixed_numbers)
            else:
                pool_size = 32 - len(fixed_numbers)
            
            remaining_size = ticket_size - len(fixed_numbers)
            
            if remaining_size > 0 and pool_size >= remaining_size:
                total_combos = comb(pool_size, remaining_size)
                
                # تحذير إذا كان البحث الشامل سيستغرق وقتاً طويلاً
                if exhaustive_search and total_combos > 1000000:
                    st.warning(f"""
                    ⚠️ **تحذير**: عدد التركيبات الممكنة = {total_combos:,}
                    
                    البحث الشامل قد يستغرق وقتاً طويلاً جداً.
                    
                    **الخيارات:**
                    - انتظر (قد يستغرق دقائق أو ساعات)
                    - أضف المزيد من الأرقام الثابتة لتقليل التركيبات
                    - استخدم البحث الذكي بدلاً من الشامل
                    """)
                    
                    if not st.checkbox("أفهم وأريد المتابعة"):
                        st.stop()
            
            with st.spinner("🔍 جاري البحث عن التذاكر المثالية..."):
                tickets = []
                ...
                for i in range(num_tickets):
                    ...
                    if exhaustive_search:
                        ...
                        def progress_update(checked, total):
                            progress_bar.progress(min(checked / total, 1.0))
                            status_text.text(f"🔍 فحص التركيبة {checked:,} من {total:,}...")
                        
                        ticket = generator.generate_ticket_exhaustive(
                ...
                if tickets:
                    ...
                    if tickets:
                        ...
                        st.info(f"""
                        📊 **إحصائيات التذاكر المولدة:**
                        - متوسط المجموع: {avg_sum:.1f}
                        - متوسط الظلال: {avg_shadows:.1f}
                        - جميع التذاكر تطابق شروطك بدقة ✅
                        """)
                else:
                    if exhaustive_search:
                        st.error("❌ لم يتم العثور على أي تذاكر تطابق الشروط")
                        st.warning("""
                        **تم فحص جميع التركيبات الممكنة!**
                        
                        النتيجة: الشروط التي حددتها **مستحيلة رياضياً**.
                        
                        **الحلول الممكنة:**
                        1. تخفيف قيد واحد على الأقل من الشروط
                        2. زيادة الحد الأقصى للظلال بمقدار 1
                        3. زيادة نسبة التسامح في المتوسط المستهدف
                        4. إلغاء تفعيل "تجنب المتتاليات"
                        
                        💡 جرب تغيير شرط واحد فقط وأعد المحاولة.
                        """)
                    else:
                        st.error("⚠️ لم يتم العثور على تذاكر في البحث السريع")
                        st.warning("""
                        **جرب أحد هذه الحلول:**
                        1. ✅ فعّل "البحث الشامل" للبحث في جميع الاحتمالات
                        2. قلل صرامة الشروط قليلاً
                        3. زد الحد الأقصى للظلال بمقدار 1
                        
                        💡 البحث الشامل سيجد التذاكر إن كانت موجودة!
                        """)
        ...
        if st.session_state.get('generated_tickets_session'):
            ...
            for idx, ticket in enumerate(st.session_state['generated_tickets_session'], 1):
                with st.container():
                    ...
                    st.markdown(numbers_html, unsafe_allow_html=True)
                    st.caption(f"المجموع: {sum(ticket)} | السعر: {LotteryConfig.TICKET_PRICES[len(ticket)]} د")
                    st.divider()
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.get('data_loaded_automatically') and st.session_state.history_df is None:
            with st.spinner("جاري التحميل التلقائي..."):
                df, msg = load_data_automatically()
                if df is not None:
                    st.session_state.history_df = df
                    st.session_state.analyzer = AdvancedAnalyzer(df)
                    st.session_state.generator = SmartGenerator(st.session_state.analyzer)
                    st.session_state.data_loaded_automatically = True
                    st.success(f"✅ تم التحميل تلقائياً: {len(df)} سحب")
                else:
                    st.warning(f"⚠️ {msg}")
    ...
    if st.session_state.history_df is None:
        st.warning("⚠️ يرجى التأكد من وجود ملف 249.xlsx في نفس المجلد")
        st.info("""
        ### 📋 المتطلبات:
        - ملف Excel بإسم `249.xlsx` في نفس مجلد التطبيق
