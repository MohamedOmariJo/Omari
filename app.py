...
def load_from_github(url: str = None) -> Tuple[Optional[pd.DataFrame], str]:
    ...
    try:
        if url is None:
            url = LotteryConfig.DATA_FILE
            # إذا كان URL GitHub، استخدمه مباشرة
            if not url.startswith('http'):
                # تحويل إلى raw.githubusercontent.com URL
                # مثال: https://raw.githubusercontent.com/username/repo/main/249.xlsx
                url = "https://raw.githubusercontent.com/MohamedOmariJo/omari/main/249.xlsx"
...
class SmartGenerator:
    ...
    def generate_ticket(
        ...
        # المرحلة 4: توليد أساسي (نادر جداً)
        candidate = list(fixed_numbers) + random.sample(pool, remaining_size)
        candidate.sort()
        return candidate
