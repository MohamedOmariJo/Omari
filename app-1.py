        ...
        for num in range(LotteryConfig.MIN_NUM, LotteryConfig.MAX_NUM + 1):
            ...
            if appearances:
                self.gaps[num] = {
                    ...
                    'avg_gap': np.mean(np.diff(appearances)) if len(appearances) > 1 else 0,
...
class SmartGenerator:
    ...
    def generate_ticket(
        ...
        attempts = 0
        max_attempts = LotteryConfig.STRICT_SHADOW_ATTEMPTS if max_shadow < 3 else LotteryConfig.MAX_GENERATION_ATTEMPTS
...
def render_smart_generator(analyzer: AdvancedAnalyzer, generator: SmartGenerator):
    ...
    with col_settings:
        ...
        max_shadow = st.slider("الحد الأقصى للظلال", 0, 5, 3)
        ...
        if st.button("🎲 توليد", type="primary", use_container_width=True):
            with st.spinner("جاري التوليد..."):
                ...
                if tickets:
                    ...
                    st.error("فشل التوليد - جرب إعدادات مختلفة")
...
def render_deep_analytics(analyzer: AdvancedAnalyzer):
    ...
    with analysis_tabs[1]:
        ...
        for pair, count in top_pairs:
            ...
            st.write(f"{pair[0]}, {pair[1]} → {count} مرة ({percentage:.1f}%)")
        ...
        for triple, count in top_triples:
            percentage = (count / analyzer.total_draws) * 100
            st.write(f"{triple[0]}, {triple[1]}, {triple[2]} → {count} مرة ({percentage:.1f}%)")
    ...
    with analysis_tabs[2]:
        ...
        fig = px.bar(
            ...
            x='رقم',
            y='آخر ظهور',
            title='الأرقام التي لم تظهر منذ أطول فترة'
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with analysis_tabs[3]:
        ...
        fig.add_vline(x=mean_sum, line_dash="dash", line_color="red", annotation_text="المتوسط")
        st.plotly_chart(fig, use_container_width=True)
